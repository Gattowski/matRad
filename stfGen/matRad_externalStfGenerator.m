classdef matRad_externalStfGenerator < matRad_StfGeneratorBase
    ...
    methods (Access = protected)
        ...
        function stf = generateSourceGeometry(this, ct, cst)
            stf = generateSourceGeometry@matRad_StfGeneratorBase(this, ct, cst);

            

            % prepare structures necessary for particles
            this.SAD = machine.meta.SAD;
            ...
            for i = 1:length(pln.propStf.gantryAngles)
                ...
                coordsAtIsoCenterPlane(:,1) = (rot_coords(:,1)*this.SAD)./(this.SAD + rot_coords(:,2));
                coordsAtIsoCenterPlane(:,2) = (rot_coords(:,3)*this.SAD)./(this.SAD + rot_coords(:,2));
               ...
               maxCtResolution = max([ct.resolution.x ct.resolution.y ct.resolution.z]);
                ...
                stf(i).sourcePoint_bev = [0 -this.SAD 0];
                ...
                for j = 1:stf(i).numOfRays
                    ...
                    if strcmp(pln.radiationMode,'photons')
                        stf(i).ray(j).beamletCornersAtIso = [this.rayPos(j,:) + [+stf(i).bixelWidth/2,0,+stf(i).bixelWidth/2];...
                            this.rayPos(j,:) + [-stf(i).bixelWidth/2,0,+stf(i).bixelWidth/2];...
                            this.rayPos(j,:) + [-stf(i).bixelWidth/2,0,-stf(i).bixelWidth/2];...
                            this.rayPos(j,:) + [+stf(i).bixelWidth/2,0,-stf(i).bixelWidth/2]]*rotMat_vectors_T;
                        stf(i).ray(j).rayCorners_SCD = (repmat([0, machine.meta.SCD - this.SAD, 0],4,1)+ (machine.meta.SCD/this.SAD) * ...
                            [this.rayPos(j,:) + [+stf(i).bixelWidth/2,0,+stf(i).bixelWidth/2];...
                            this.rayPos(j,:) + [-stf(i).bixelWidth/2,0,+stf(i).bixelWidth/2];...
                            this.rayPos(j,:) + [-stf(i).bixelWidth/2,0,-stf(i).bixelWidth/2];...
                            this.rayPos(j,:) + [+stf(i).bixelWidth/2,0,-stf(i).bixelWidth/2]])*rotMat_vectors_T;
                ...
                for j = stf(i).numOfRays:-1:1
                    ...
                    this.initializeEnergy(this,ct,cst)
                end
                ...
                this.initializePostProcessing(this,ct,cst)
        ...
        function initializeEnergy(this,ct,cst)
            throw(MException('MATLAB:class:AbstractMember','Abstract function initializeEnergy of your StfGenerator needs to be implemented!'));
        end

        function initializeIonPostProcessing(this,ct,cst)
            throw(MException('MATLAB:class:AbstractMember','Abstract function initializePostProcessing of your StfGenerator needs to be implemented!'));
        end
    end
end
